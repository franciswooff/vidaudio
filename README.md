...
3.	Prepare the video & audio files you will use in your test. The project is set up to work with .mp4 video files & .wav audiofiles. Other file formats are possible, the code downloaded in step 2 can be adapted to accommodate.  
...
There are number of other features of the test which can be configured via lines 5 to 11, but ignore these for now.
...
Congratulations! You should have a working test which you can use with no further steps* Hopefully the format the results arrive in via e-mail is self-explanatory & easy to bring into e.g. Excel: paste the results into a text editor such as Notepad, save the file as a .csv file (rather than .txt) & then open with Excel. The following steps provide additional functions which you may use, but do not have to:  
\* note however, that if you are not using labels or custom text on test pages, as described in steps 15 & 16, you will need to delete the example files in the “labels” & “extras” folders, detail (if needed) is provided as step 17.

8.	**To add an additional reference audio track to a page of the test** - list the pages (where you want a reference track to appear) on line 5 of the EDITME.txt file. The format is Comma Separated Values.
Example: (regardless of how many pages you have) if you want to have references on pages 2, 5 & 6 your line 5 would be:  
2,5,6  
...
9.	**To turn off randomisation of linking audio tracks to rating sliders for specified pages** - by default the test pages randomise the order in which your audio tracks for that page/video are linked to the rating slider/button pairs. If you wish to turn this randomisation off for any given page(s) (typically where you are providing a "training page" & so need the audio tracks to map consistently to the slider/button pairs) list the pages on line 6 of the EDITME.txt file. The format is Comma Separated Values. Example: (regardless of how many pages you have) if you want to have "training pages" on pages 1 & 4 your line 6 would be:  
1,4

10.	**To turn off randomisation of the order in which your video & associated audio files are presented to the subject to rate** - by default (i.e. with the EDITME.txt file as first downloaded) your test videos/pages are presented to the subject in a random order. To switch this off clear line 7 of the EDITME.txt file (any character on line 7 will enable randomisation, an empty line will switch it off).

11.	**To turn on autoplay** - using this setting video & audio files MAY autoplay (some browsers suppress autoplay, as of July 22 Chrome allows it, Firefox requires permission granting per site). Any character on line 8 will enable autoplay, an empty line will switch it off.  
Autoplay of media should be used with caution (asking user consent first is good practice).

12.	**To turn off looping** - by default (i.e. with the EDITME.txt file as first downloaded) your video & audio files will loop back to the beginning & play again on ending. To switch this off clear line 9 of theEDITME.txt file (any character on line 9 will enable looping, an empty line will switch it off).

13.	**To alter the opacity of the reference track slider** - by default (i.e. with the EDITME.txt file as first downloaded) the reference track slider (where one is present) is partly transparent. This is set by the 0.5 on line 10 of EDITME.txt. The range is from 0 (invisible) to 1 (fully opaque) & you can alter opacity by editing line 10.

14.	**To alter the default level of ratings sliders** - by default (i.e. with the EDITME.txt file as first downloaded) ratings sliders are set to an initial position in the middle. This is set by the 50 on line 11 of EDITME.txt. The slider range is from 0 (bottom) to 100 (top) & you can set another initial position by editing line 11. Note that for the slider for a reference track (where one is present) this position is the fixed position.

15.	**To add a scale label to your sliders** - to add a scale to the sliders on a particular page place a text file in the folder “labels”. A scale label for your first test page would be “1.txt”, for the third page would be “3.txt” etc. (no issue for a page to have no label text file allocated).  
The label format is 5 rows, but you can use whichever of these you like - so you could use only the top & bottom rows to give labels to each end of the slider, top, middle & bottom for a 3 point scale etc. Each label is a separate line in the text file & you need to leave lines blank where you aren’t using a 5 point scale. Top & bottom labels only should have 3 blanks lines between them. A 3 point scale would be top label on the first line, blank line, middle label on the third line, blank line, bottom label on the fifth line. 5 point scale = lines 1-5 of the text file, with the top label on the first line.  
The labels folder in your downloaded project folder contains example labels for pages 1 & 2. Check the format of these as a guide. You will need to either delete these files if you are not using labels on pages 1 & 2 or edit them to your labels if you are.

16.	**To include extra custom text on any test page** - similar to 15. above. To add additional text, appearing as a paragraph (or paragraphs) below the standard instructions on a particular page place a text file in the folder “extras”. The text for your first test page would be “1.txt”, for the third page would be “3.txt” etc. (no issue for a page to have no text file allocated).  
...
The extras folder in your downloaded project folder contains example paragraphs for pages 2 & 3. Check the format of "3.txt" for examples of line break & paragraph break. You will need to either delete these files if you are not using extra text on pages 2 & 3 or edit them to your text if you are.  

17.	**If you are not using labels or custom text on your test pages** you will need to clear the examples provided. This is best done by deleting the text files in the “labels” & “extras” folders (leaving those folders empty). If you prefer to delete the folders altogether this is fine too, but you’ll need to recreate them if you change your mind (a good middle ground would be to keep the local files & folders in your downloaded project folder, but not have them on the server).

18.	**Further customisation of text on the pages of your test** (the actual test page or the start & end pages) is also possible. This means venturing a little into the world of html text. For an introduction & reference see https://www.w3schools.com/html  
You don’t need a specialist code editing application to do this, you can edit with Notepad on Windows or TextEdit on Mac OS. It may be easier with something that colour codes the different parts of the code though. As well as code editing applications your Microsoft OneDrive will display the code like this if you view & edit the files online (via a web browser).  
...
Editing the code of the test page “page.php” & the end page “end.php” is trickier, as php is more likely to “break” than html. To edit the standard instructions of the test page edit between the \<p> & \</p> tags on lines 100-102. If you wish to remove the standard instructions altogether delete lines 100-102. Note that in customising this way you are customising every test page, any customisation to an individual page’s instructions should be by the method in step 16.  
To edit the standard message on the end page either customise or replace the text between the \<p> & \</p> tags on line 13.  

19. **Adding a second test with different parameters to directly follow the initial test** - could be by placing the second test in a subfolder of your project folder/public_html & following details in step 18 above to include text & a link on your first test end page to direct the test subject to the second test.  
E.g. edit the "Thanks" paragraph (line 13) to \<p>There now follows a second part to the test, please click \<a href="test2/">this link\</a> (you will need to enter your participant number again)\</p> (test2 used as an example for the name of the folder containing your second test). Edit the subject line of the email you receive (line 36 of the end page) to distinguish results for the second test from the first.  

20.	**To extensively modify the test** - for example to swap the use of videos for still images or work with a 7 point slider scale - either follow the approach of step 18 (the pages likely to be concerned are “page.php” & “preview.php”) or if you are looking for something where the customisation is more to individual pages you might set the test up as closely as possible to what you want, copy the code of the pages to be changed (right click & “view source” in a browser), save these as new html files, perform the edits & incorporate them into your test.

It is likely at time of most recent update (Sept 2022) that further development of this project as a repository on GitHub will be paused in favour of investigating converting the project to an Azure application. Of the list below of functionality/features to be added the most important are done (strike through text), item 9 may be difficult to implement as an option (it would be easy to convert the test to make participant number mandatory in every case), item 10 wil be investigated, but early signs are audio/video sync looks already to be operating at less than 5hz, slower may not be desirable:
...
~~results mail can be formatted differently (order or separators) if preferred~~