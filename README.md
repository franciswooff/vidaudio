...
There are number of other features of the test which can be configured via lines 4 to 9, but ignore these for now.
...
It is likely at time of most recent update (Sept 2022) that further development of this project as a repository on GitHub will be paused in favour of investigating converting the project to an Azure application. Of this list of functionality/features to be added the most important are done (strike through text), item 9 may be difficult to implement as an option (it would be easy to convert the test to make participant number mandatory in every case), item 10 wil be investigated, but early signs are audio/video sync looks already to be operating at less than 5hz, slower may not be desirable:
