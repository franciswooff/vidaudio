...
8.	**To add a an additional reference audio track to a page of the test** - list the pages (where you want a reference track to appear) on line 5 of the EDITME.txt file. The format is Comma Separated Values.
...
Example: (regardless of how many pages you have) if you want to have "training pages" on pages 1 & 4 your line 6 would be:  
1,4

10.	**To turn off randomisation of of the order in which your video & associated audio files are presented to the subject to rate** - by default (i.e. with the EDITME.txt file as first downloaded) your test videos/pages are presented to the subject in a random order. To switch this off clear line 7 of theEDITME.txt file (any character on line 7 will enable randomisation, an empty line will switch it off).
...
It is likley at time of most recent update (Sept 2022) that further developemnt of this project as a repository on GitHub will be paused in favour of investigating converting the project to an Azure application. Of this list of functionality/features to be added the most important are done (strike through text), item 9 may be difficult to implement as an option (it would be easy to convert the test to make participant number mandatory in every case), item 10 wil be investigated, but early signs are audio/video sync looks already to be operating at less than 5hz, slower may not be desirable:
